...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, n_gram=1, remove_subwords = True, batch_size=256, device='cuda:0'):
    preds = []
    for batch_start in range(0, len(refs), batch_size):
        ...
        for i in range(len(ref_lens)):   
            if remove_subwords:
                ...
                ref_ids = [k for k, w in enumerate(ref_tokens[i]) if w not in set(string.punctuation)]
                hyp_ids = [k for k, w in enumerate(hyp_tokens[i]) if w not in set(string.punctuation)]                
