...
class EvalHTTPProxy(Process):
    ...
    def create_flask_app(self):
        try:
            ...
            from bert_serving.client import ConcurrentEvalClient
        except ImportError:
            raise ImportError('EvalClient or Flask or its dependencies are not fully installed, '
                              'they are required for serving HTTP requests.'
                              'Please use "pip install -U bert-serving-server[http]" to install it.')

        # support up to 10 concurrent HTTP requests
        bc = ConcurrentEvalClient(max_concurrency=self.args.http_max_connect,
